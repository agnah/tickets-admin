...
const UserDetail2 = ({ user, datos }) => {
  ...
  const rolSelect =
    user.rolUsuario === ADMIN || user.rolUsuario === DIOS
      ? (optionListRol = ["admin", "editor", "lector"])
      : (optionListRol = ["editor", "lector"]);
  const perfilSelect =
    user.rolUsuario === ADMIN || user.rolUsuario === DIOS
      ? (optionListPerfil = ["administrador", "tecnico"])
  ...
  const idSector = (sector) => {
    const entry = Object.entries(areaMapping).find(
      ([, value]) => value === sector
    );
  ...
  const sendPatchRequest = async (formData) => {
    try {
      ...
      setMessage("Se modifico el usuario correctamente.")
      setShow(true)
    } catch (error) {
          ...
          <article className="row">
            <div className="col-lg-4 col-md-4 col-sm-12">
              <p className="d-flex">
                <label
                  htmlFor=""
                  className="user-detail-label d-flex align-items-center"
                >
                  Área:
                </label>
                <Select
